...
//use tattali\CalendarBundle\Calendar;
use Symfony\Component\Routing\Annotation\Route;
use Symfony\Component\HttpFoundation\JsonResponse;
//use CalendarBundle\CalendarBundle\Calendar;
...
{
    #[Route('/index', name: 'app_culture_index', methods: ['GET'])]
    ...
    #[Route('/pag', name: 'app_culture_indexx')]
    ...
    public function barChartAction( CultureRepository $cultureRepository ): Response
    {
        $results= $cultureRepository->chartRepository();
        $totalCount = array_reduce($results, function($carry, $result) {
            return $carry + $result['quantite'];
        }, 0);

        foreach ($results as $result) {
            $percentage = round(($result['quantite']  ));
            $obj = new Pie;
        ...
        return $this->render('culture/chart.html.twig', array(
            'results'  =>  $resultArray,
        ));
    }
    ...
    public function delete(Request $request, Culture $culture, CultureRepository $cultureRepository): Response
    {
        if ($this->isCsrfTokenValid('delete'.$culture->getId(), $request->request->get('_token'))) {
            $relatedEntities = $culture->getTerrain(); // Assuming "Produit" is the related entity
            if ($relatedEntities->count() > 0) {
                return new Response('Cette catégorie ne peut pas être supprimée car elle est liée à un ou plusieurs produits');
            }
        }
        return $this->redirectToRoute('app_culture_index', [], Response::HTTP_SEE_OTHER);
    }
   ...
   // #[Route('/{id}/calendrier', name: 'app_culture_calendrier')]
   

      //  public function myAction(CalendarEvent $calendarEvent)
    //{
     //   $event = new CalendarEvent(
      ...
      //  $calendar = $this->get('calendar')->getCalendar($event->getStart(), $event->getEnd());

      //  return $this->render('my/view.html.twig', [
      //      'calendar' => $calendar
       // ]);
    //}
